...
12. **Parent & Relation Membership Attributes (`parent_osm_id`, `parent_feature_kind`, `relation_id`, `member_role`, `access_type`)**:
    ...
    * Relations matching `type IN ('site', 'parking', 'building')` are pre-filtered via `osmium tags-filter` into an in-memory OPL index and joined deterministically (prioritizing `parking` > `site` > `building`).
