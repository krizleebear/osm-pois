...
# Usage: /app/entrypoint.sh <input.osm.pbf> <output.places.parquet> [country_code]
...
if [ -z "$INPUT_PBF" ] || [ -z "$OUTPUT_PARQUET" ]; then
    ...
    echo ""
    echo " Usage:  /app/entrypoint.sh <input.osm.pbf> <output.places.parquet> [country_code]"
    echo "****************************************************************"
    exit 1
fi
